...
class Message extends ActiveRecord{
	...
	public function getLatestMessage($n){
		$ret = $this->find()->orderBy('id DESC')->limit($n)->all();
		return $ret;
	}

	public function getUnHandledProject(){
		$ret = $this->find()->where(['type' => self::MESSAGE_TYPE_PROJECT])->all();
	}
	
	public function getUnHandledOrder(){
		$ret = $this->find()->where(['type' => self::MESSAGE_TYPE_ORDER])->all();
	}
	...
	public function createMessage($data,$type){
		...
		$projectId = isset($data['project_id'])? $data['project_id'] : -1;
		$orderId = isset($data['order_id'])? $data['order_id'] : -1;
